...
.equ MCUCR_pint0r =	0 << SE |	0 << SM |	0 << ISC11 |	0 << ISC10 |	1 << ISC01 |	1 << ISC00
		...
		; ошибка в битах конф.[-] чтобы помен€лс€ режим нужно как-то перезагрузить прерывпни€?
;====================================================================
; VARIABLES
;====================================================================
.def temp = r16
...
reti
	int0_release:
		;inc setStatus	; что-то сделать с этим: задержка откладываетс€ до выполнени€ задачи mode
		;; if bmode is 0, mode = 1
		;cpi bmode, 1	; bmode is 1?
		;sbrc SREG, Z	; bit1 in sreg
		;ldi mode, 1	; turning on led for 3s (25min)
		;; if bmode is 2, mode = 2
		;cpi bmode, 1
		;sbrc SREG, Z	; bit1 in sreg
		;ldi mode, 2	; turning on led for 1s (5min)
		;...etc. (x4)
...
TIM_OVF0: ;________________________________
	...
	;push r5		; перенесено выше, т.к. в прот.случае придетс€ повтор€ть в ветках
